...
void OllamaBotRandomChatter::OnUpdate(uint32 diff)
{
    ...
    MaybeSaveRumorsToDB(); // internally throttled to the same cadence
...
void OllamaBotRandomChatter::HandleRandomChatter()
{
    ...
    for (auto const& itr : allPlayers)
    {
        ...
        for (Player* realPlayer : realPlayers)
        {
            ...
            {
                nearRealPlayer = true;
                RumorRecordSighting(bot, realPlayer); // rumor system: bot remembers seeing them
                break;
            }
        }
        ...
        processedBotsThisTick.insert(guid);
            ...
            auto prompt = [bot, &environmentInfo]()
            {
                ...
                std::string personalityPrompt   = GetPersonalityPromptAddition(personality) + GetRumorPromptText(bot) + GetSeekerJourneyPromptText(bot);
