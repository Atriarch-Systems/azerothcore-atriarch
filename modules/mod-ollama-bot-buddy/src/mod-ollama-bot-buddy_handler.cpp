...
bool BotBuddyChatHandler::OnPlayerCanUseChat(Player* player, uint32_t type, uint32_t lang, std::string& msg)
{
    ProcessChat(player, type, lang, msg, nullptr);
    return true;
}
bool BotBuddyChatHandler::OnPlayerCanUseChat(Player* player, uint32_t type, uint32_t lang, std::string& msg, Group* /*group*/)
{
    ProcessChat(player, type, lang, msg, nullptr);
    return true;
}
bool BotBuddyChatHandler::OnPlayerCanUseChat(Player* player, uint32_t type, uint32_t lang, std::string& msg, Channel* channel)
{
...
{
    LOG_DEBUG("server.loading", "ProcessChat: sender={} type={} lang={} msg='{}' channel={}",
