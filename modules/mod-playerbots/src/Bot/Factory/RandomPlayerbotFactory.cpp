...
Player* RandomPlayerbotFactory::CreateRandomBot(WorldSession* session, uint8 cls, std::unordered_map<NameRaceAndGender, std::vector<std::string>>& nameCache)
{
    ...
    // Weighted gender selection based on AiPlayerbot.RandomBotFemaleRatio (default 0.5 for legacy behavior)
    const float femaleRatio = std::clamp(sPlayerbotAIConfig.randomBotFemaleRatio, 0.0f, 1.0f);
    const uint8 gender = (frand(0.0f, 1.0f) < femaleRatio) ? GENDER_FEMALE : GENDER_MALE;
