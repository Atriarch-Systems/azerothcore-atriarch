...
# Materialize module .conf files from their .dist templates: this core's
# ConfigMgr loads only modules/<name>.conf (no .dist fallback), so without
# this every module runs on compiled-in defaults.
if [[ -d "$CONF_DIR/modules" ]]; then
    for dist in "$CONF_DIR"/modules/*.conf.dist; do
        [[ -f "$dist" ]] && cp -vn "$dist" "${dist%.dist}"
    done
fi
